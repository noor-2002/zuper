...
function getUserActivitiesById(userId, page, pageSize, month, callback) {
    ...
    db.all(`
        ...
        JOIN activity a ON ca.act_id = a.act_id
        WHERE ca.user_id = ? AND strftime('%m', ca.created_at) = ?
        LIMIT ? OFFSET ?
    `, [userId, month, pageSize, offset], callback);
}
...
function addCompletionActivity(userId, actId, callback) {
    db.get('SELECT points FROM activity WHERE act_id = ?', [actId], (err, row) => {
        ...
        db.serialize(() => {
            ...
            db.run('INSERT INTO completion_activity (user_id, act_id, points_earned) VALUES (?, ?, ?)', [userId, actId, points]);
            db.run('UPDATE users SET total_points = total_points + ? WHERE user_id = ?', [points, userId]);
...
app.get('/singleuser/:userId', (req, res) => {
    ...
    const month = req.query.month || (new Date().getMonth() + 1).toString().padStart(2, '0');
    getUserActivitiesById(userId, page, pageSize, month, (err, rows) => {
        if (err) {
            console.error('Error executing query:', err);
            res.status(500).send({ status: 'failure', message: 'Server error' });
            return;
...
app.post('/add-completion-activity', (req, res) => {
    ...
    addCompletionActivity(userId, actId, (err) => {
        if (err) {
            console.error('Error adding completion activity:', err);
            res.status(500).send({ status: 'failure', message: 'Server error' });
            return;
        }
        res.status(201).send({ status: 'success', message: 'Logs added and total count is updated' });
    });
});
